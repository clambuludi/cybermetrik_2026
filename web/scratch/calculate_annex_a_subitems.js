...
const rows = cmDb.prepare("SELECT id, id_norma, dominio, pregunta FROM preguntas WHERE activo = 1").all();
cmDb.close();

// Filter Annex A sub-items (they have id_norma starting with A. and have a suffix like .a, .b, .c)
const SUB_ITEM_REGEX = /^(.+?\d+)\.?([a-z])$/;
const annexASubitems = rows.filter(r => r.id_norma.startsWith('A.') && r.id_norma.match(SUB_ITEM_REGEX));
...
console.log("=== CALCULATING DIRECTLY BY ANNEX A SUB-ITEMS ===");
console.log("Total Annex A Sub-items in DB:", annexASubitems.length);
