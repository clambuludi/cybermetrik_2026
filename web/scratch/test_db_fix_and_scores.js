...
try {
  cmDb.transaction(() => {
    ...
    console.log("Applied question text fixes in transaction.");
    ...
    // We also need the slug mapping.
    // Let's generate a temporary mapping dictionary.
    // Explicit oldSlug to id_norma map
    ...
    // Build standard catalog mapping (same as generate_hardcoded_slug_map)
            ...
            if (!newSlug && norma.startsWith('ISO-')) {
                ...
                if (numPart >= 1 && numPart <= 12) {
                    const mapIsoToActive = {
                        1: 'Clausula 4.a', 2: 'Clausula 4.b', 3: 'Clausula 4.c', 4: 'Clausula 4.d',
                        5: 'Clausula 5.a', 6: 'Clausula 5.b', 7: 'Clausula 5.c',
                        8: 'Clausula 6.a', 9: 'Clausula 6.a', 10: 'Clausula 6.b', 11: 'Clausula 6.d', 12: 'Clausula 6.e'
                    };
                    newSlug = currentSlugsMap[mapIsoToActive[numPart]];
                }
            }
            if (newSlug) {
    ...
    // Group active questions into sections like layout.tsx
    ...
    // Calculate score function (like calcularPuntajesConsistentes)
              ...
              if (isClausesSection) {
                ...
                if (!progress.ignored[itemId]) {
                  ...
                  if (numericVal === 1.0 || numericVal === 0.5) {
                    if (hasDriveLink) clausesPointsSum += pValue;
                    else clausesPointsSum += pValue * 0.4;
                  }
                }
              }
            } else {
    ...
    console.log("Calculated Mapped Scores with DB Fixed:", scores);

    // Calculate individual ISO groups scores (A5, A6, A7, A8)
    const allItems = [];
    sections.forEach(sec => {
      if (sec.title !== 'Cláusulas ISO 27001') {
        allItems.push(...sec.checklist.map(item => ({ ...item, sectionTitle: sec.title })));
      }
    });
    ...
    // FORCE ROLLBACK so we don't apply the changes yet
